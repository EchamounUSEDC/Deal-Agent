...
| 🎓 Sales School | `pages/sales_school.py` + `utils/research.py` | **The AI's training ground**: a tiered research library (thin-slicing, SPIN, OARS, trust theory, speech-rate & interruption science, FINRA 2210/2111, advisor pain-point playbooks) drives a 20-category confidence-weighted scoring engine with a compliance **gate** that overrides scores. Tabs: how the AI coaches, score-a-call lab, objection roleplay with research-cited feedback **plus the recorded Logan/Ryan mock-call series** (listen to the real call, then run it yourself — the AI plays Logan), personalized curriculum with per-week case-study listening assignments, quizzes, playbook. Full source doc preserved in `docs/research_library.md`; call audio in `assets/mock_calls/` |
